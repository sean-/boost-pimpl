...
#include "boost/pimpl/pimpl_constructors.hpp"
#include "boost/pimpl/safebool.hpp"
...
class pimpl<T>::pimpl_base
{
    ...
    typedef boost::safebool<T>           safebool;
    ...
    /// @details Functions are protected. They only allow access to the underlying
    /// data by the derived classes and only inside implementation files (where
    /// pimpl<>::implementation is visible) as follows:
    /// @code
    ...
    /// still allows to modify the underlying data. Instead, Pimpl behaves like a
    /// proxy. Therefore, 'const' instances return 'const' pointers and references.
    //@{
